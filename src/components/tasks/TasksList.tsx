...
export const TasksList: React.FC = () => {
    ...
    const reOrderTasks = useCallback(
        (tasks: Task[]): Task[] => {
            // Seperate tasks into pinned and unpinned
            ...
            <TasksContainer>
                ...
                {multipleSelectedTasks.length > 0 && (
                    <SelectedTasksContainer>
                        <div>
                            <h3>
                                <RadioButtonChecked /> &nbsp; Selected {multipleSelectedTasks.length} Task{multipleSelectedTasks.length > 1 ? "s" : ""}
                            </h3>
                ...
                {search && reOrderTasks(user.tasks).length > 1 && user.tasks.length > 0 && (
                    ...
                    >
                        <b>
                            Found {reOrderTasks(user.tasks).length} task{reOrderTasks(user.tasks).length > 1 ? "s" : ""}
                        </b>
                    </div>
                )}
                {user.tasks.length !== 0 ? (
